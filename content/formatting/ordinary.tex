...
\paragraph{Parsing} transforms an array of tokens to a \textit{parse tree} (also known as
...
\begin{figure}\label{fig:ifStmtAst}
  ...
  \begin{forest}
    ...
    [  {BlockStmt}, edge label={node[midway,right]{\scriptsize \code{consequent}}}
    [   {ExprStmt}, edge label={node[midway,left]{\scriptsize \code{body[0]}}}
    [    {AssignmentExpr (op: \code{=})}, edge label={node[midway,left]{\scriptsize \code{expression}}}
    [     {Identifier (name: \code{c})}, edge label={node[midway,left]{\scriptsize \code{left}}}]
    [     {BinaryExpr (op: \code{+})}, edge label={node[midway,right]{\scriptsize \code{right}}}
    [      {Identifier (name: \code{a})}, edge label={node[midway,left]{\scriptsize \code{left}}}]
    [      {BinaryExpr (op: \code{*})}, edge label={node[midway,right]{\scriptsize \code{right}}}
    [       {Literal (value: \code{2})}, edge label={node[midway,left]{\scriptsize \code{left}}}]
    [       {Identifier (name: \code{b})}, edge label={node[midway,right]{\scriptsize \code{right}}}]]]]]]]]
  \end{forest}
...
\subparagraph{Hard line breaks} force the insertion of a line break between two tokens.
