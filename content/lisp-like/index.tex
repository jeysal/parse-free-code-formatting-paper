...
% TODO soft line breaks: maybe break after prefix is disallowed by language
% or we just don't want it because it looks terrible
% => introduce break allowance based on our spacing and treat as single token
% TODO same thing for parens, those would otherwise look bad and
% more importantly inconsistent (break sometimes before, sometimes after)

% TODO what if indentation and/or single tokens are too large?
% 1) limit indentation (meh, also does not solve problem if token is just too large)
% 2) print offending token and bump line size to its end
